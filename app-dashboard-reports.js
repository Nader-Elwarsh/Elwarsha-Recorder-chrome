...
function renderDash(){let el=document.getElementById("dashboard");if(!el)return;let c=arr(K.c),d=arr(K.d),r=arr(K.r),p=arr(K.p);let today=dayKeyLocal(new Date());let openOrders=r.filter(x=>x.status!=="مكتمل"&&x.status!=="ملغي");let todayOrders=openOrders.filter(x=>x.visit&&dayKeyLocal(x.visit)===today);let overdue=openOrders.filter(x=>x.visit&&dayKeyLocal(x.visit)<today);let staleOrders=openOrders.filter(x=>typeof requestIsStale==="function"&&requestIsStale(x));let overdueAlertDays=+settings().overdueAlertDays||7;let waitingParts=openOrders.filter(x=>x.partsWaiting);let inWorkshop=openOrders.filter(x=>x.executionPlace==="الورشة"||(x.workshopStatus&&x.workshopStatus!=="غير مطلوب"&&x.workshopStatus!=="تم التسليم"));let low=p.filter(x=>+x.qty<=+x.min).length;let ym=new Date().toISOString().slice(0,7);let monthOrders=r.filter(x=>(x.closedAt||x.createdAt||"").slice(0,7)===ym);let monthClosed=monthOrders.filter(x=>x.closed);let monthLabor=monthClosed.reduce((a,x)=>a+(+x.labor||0),0);let monthRevenue=monthLabor+monthClosed.reduce((a,x)=>a+(+x.partsTotal||0),0);let unpaidRemain=r.filter(x=>!x.closed).reduce((a,x)=>a+Math.max(0,(+x.total||0)-(+x.deposit||0)),0);let newCustomers=c.filter(x=>(x.createdAt||"").slice(0,7)===ym).length;let tasksToday=taskRows().filter(x=>!x.completed&&x.date===today).length;let pendingCallsCount=arr(K.pc).length;let needsAttention=overdue.length+staleOrders.length+waitingParts.length+low+pendingCallsCount;el.innerHTML=`
